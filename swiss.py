...
def ValidatePairings(pairings: Pairings, n: Optional[int] = None) -> None:
    ...
    for p, q in pairings:
        ...
        if p.id in q.opponents or q.id in p.opponents:
            ...
            warnings.warn(
                f"{p.id} vs. {q.id} from {matches_ago} matches ago.", RepeatMatchWarning
            )
    ...
    with contextlib.redirect_stdout(stream):
        for (p, q) in pairings:
            ...
            n_stars = min(
                ...
                max(
                    0,
                    RoundTo(5 + math.log(max(0.00001, abs(p.score - q.score)), 2), 0.5),
                ),
            )
            star_string = "⬥" * int(n_stars)
            if (
                n_stars % 1 == 0.5
            ):  # Exact float comparison!? Should be OK because we just rounded to an exact power of 2.
                star_string += "⬦"
            if n_stars > 2:
                if stream.isatty():
...
class Pairer(object):
    ...
    def MaximumMatchingPairings(self):
        ...
        nodes = list(
            collections.Counter(
                {p: p.requested_matches for p in self.players}
            ).elements()
        )
        g = nx.Graph()
...
def OrderPairingsByTsp(pairings: Pairings) -> Pairings:
    ...
    for alpha in range(len(pairings)):
        ...
        for beta in range(len(pairings)):
            ...
            # normal;normal
            # swapped;swapped
            weights[alpha_right, beta_left] = weights[
                alpha_left, beta_right
...
def PairingTransitionCost(pairing_alpha, pairing_beta) -> float:
    left_cost = (
        1 - difflib.SequenceMatcher(a=pairing_alpha[0], b=pairing_beta[0]).ratio()
    )
    right_cost = (
        1 - difflib.SequenceMatcher(a=pairing_alpha[1], b=pairing_beta[1]).ratio()
    )
...
def Main(argv):
    ...
    sheet = sheet_manager.SetSheetManager(set_code, cycle, FLAGS.fetch)
    previous_set_code = list(magic_sets.names.keys())[-2]
    sheet_old = sheet_manager.SetSheetManager(previous_set_code, 5, FLAGS.fetch)
    GeneratePairings(sheet, sheet_old, cycle, FLAGS.tabprint, FLAGS.write)


def GeneratePairings(
    sheet: sheet_manager.SheetManager,
    sheet_old: sheet_manager.SheetManager,
    cycle: int,
    tabprint: bool,
    write: bool,
):
    ...
    ValidatePairings(pairings, n=pairer.correct_num_matches + bool(pairer.byed_player))
    t = time.time() - start
    try:
        os.mkdir("pairings")
    except FileExistsError:
        pass
    with open(f"pairings/{cycle}.{int(time.time())}.txt", "w") as output:
        PrintPairings(pairings, stream=output)
    with open(f"pairings/{cycle}.txt", "w") as output:
        PrintPairings(pairings, stream=output)
